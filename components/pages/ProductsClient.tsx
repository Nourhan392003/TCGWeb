...
export default function ProductsClient() {
    ...
    const getProductCounts = () => {
        if (!products) {
            return { inStock: 0, outOfStock: 0, brands: {}, types: {} };
        }
    ...
    const filteredProducts = useMemo(() => {
        ...
        return products.filter((product: any) => {
            ...
            const stockMatch =
                availableInStock === null || product.inStock === availableInStock;
                    ...
                    <aside className={`lg:w-72 flex-shrink-0 ${isFiltersOpen ? "block" : "hidden"} lg:block`}>
                        <div className="bg-[#12121a] rounded-xl border border-gray-800 p-3 sm:p-4 sticky top-4">
                            ...
                            <div className="mb-3 sm:mb-4">
                                <label className="text-xs sm:text-sm text-gray-400 mb-2 block ltr:text-left rtl:text-right">
                                    {t("searchPlaceholder").split("...")[0]}
                                </label>
                                <div className="relative">
                                    <Search
                                        className={`absolute ${isRTL ? "right-3" : "left-3"} top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500`}
                                    />
                                    <input
                                        type="text"
